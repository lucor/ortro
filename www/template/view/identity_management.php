...
if (isset($_REQUEST['filter_reset_x'])) {
    unset($_SESSION['filter_identity']);
} else {
    ...
    for ($j = 0; $j < sizeof($filter_array); $j++) {
        if (!isset($_SESSION['filter_identity'][$filter_array[$j]])) {
            $_SESSION['filter_identity'][$filter_array[$j]] = '';
        }
...
$identities = $dbUtil->dbQuery($dbh, 
                  $dbUtil->getIdentities($_SESSION['filter_identity']
                                                  ['filter_system']),
...
$f_filter_system_obj->setSelected($_SESSION['filter_identity']['filter_system']);
...
?>
<div id="ortro-title">
<?php echo ID_ADMIN_TOP; ?>
</div>
<p>
<?php echo ID_ADMIN_TITLE; ?>
</p>
