...
try {
    ...
    $sql_file  = ORTRO_ATTACHMENTS . $id_job . DS . $parameters['db_execute_sql_from_file_query'];
    ...
    try {
        ...
        $dbUtil->dbExecFromFile($dbh, $sql_file);
        $dbh    = $dbUtil->dbCloseConn($dbh);
        unset($dbh);
        
        $loggerPlugin->trace('DEBUG', 'id=' . $id_job . ' result: file_query_ok');
