...
/**
 * DB Custom Query Report.
 * Creates a report by executing a custom query against the selected database.
 *
 * PHP version 5
 *
...
try {
    ...
    $query            = $parameters['db_custom_query_report_query'];
    $attach_result_as = $parameters['db_custom_query_report_attach_result'];
    $filename         = $parameters['db_custom_query_report_file_name'];
    ...
    if (count($rows)>0) {
        ...
        $retention_data['retention']    = $parameters['db_custom_query_report_retention'];
        $retention_data['path']         = $reports_storing_path;
        $retention_data['filename']     = $filename;
        
    } else {
        $msg_exec = 'No records found.'; 
    }
    //---- End plugin code -----
