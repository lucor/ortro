...
/**
 * Oracle Free Tablespace Report.
 * Generate report on free tablespace(s).
 *
 * PHP version 5
 *
...
try {
    ...
    $attach_result_as = $parameters['db_oracle_freespa_report_attach_result'];
    $filename         = $parameters['db_oracle_freespa_report_file_name'];
    ...
    //Base query
    $query_base = "SELECT FILE_NAME, TABLESPACE_NAME, (BYTES/(1024*1024)) Mega,  " .
                  "AUTOEXTENSIBLE, INCREMENT_BY, " .
                  "round((MAXBYTES/(1024*1024)),2) MaxMega, " .
                  "round((SELECT SUM(a.bytes) FROM dba_free_space a WHERE " . 
                  "a.file_id = b.file_id)/(1024*1024),2) free " .
                  "FROM dba_data_files b ";
                    
    $query_bottom = "SELECT FILE_NAME, TABLESPACE_NAME, (BYTES/(1024*1024)), " .
                    "AUTOEXTENSIBLE, INCREMENT_BY, " .
                    "round((MAXBYTES/(1024*1024)),2), " .
                    "round((SELECT SUM(a.bytes)  " . 
                    "FROM dba_free_space a WHERE " . 
                    "a.file_id = b.file_id)/(1024*1024),2) free " .
                    "FROM dba_temp_files b order by 7 nulls first";
    
    $query = '';
    if (isset($tablespaces) && $tablespaces != '') {
        $tablespaces_array = explode(',', $tablespaces);
        for ($k = 0; $k < sizeof($tablespaces_array); $k++) {
            $query .= $query_base . $query_tablespace . 
                      " where tablespace_name = '" .  
                      trim($tablespaces_array[$k]) .  "' UNION ";
        }
    } else {
        $query .= $query_base . ' UNION ';
    }
    ...
    if (count($rows)>0) {
        ...
        $reports_storing_path = ORTRO_REPORTS . $id_system . 
                                DIRECTORY_SEPARATOR . $id_job . 
                                DIRECTORY_SEPARATOR;
        ...
        $retention_data['retention']    = $parameters['db_oracle_freespa_report_retention'];
