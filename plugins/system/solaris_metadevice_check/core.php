...
/**
 * [Solaris] metadevice Check.
 * Checks the state of the specified metadevice.
 *
 * PHP version 5
 *
...
try {
    ...
    $user       = $parameters['solaris_metadevice_check_user'];
    $port       = $parameters['solaris_metadevice_check_port'];
    $metadevice = $parameters['solaris_metadevice_check_metadevice'];
    
    $ip = $job_infos['ip'];
    
    $ssh    = new SSHUtil();
    $path   = dirname($argv[0]);
    $script = $path . '/metadevice-check.ksh';
    
    $sshCommandResult = $ssh->sshConn($user, $ip, $port, $script, true, $metadevice);
