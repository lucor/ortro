...
/**
 * Free memory.
 * Checks the amount of the free memory and 
...
try {
    ...
    $user = $parameters['mem_free_user'];
    $port = $parameters['mem_free_port'];
    
    // threshold
    $threshold = $parameters['mem_free_threshold'];
    
    $ip = $job_infos['ip'];
    
    $ssh    = new SSHUtil();
    $path   = dirname($argv[0]);
    $script = $path . '/mem-free.ksh';
