...
/**
 * [Solaris] zfs scrub check, check if all the zpool need a scrubbing.
 *
 * PHP version 5
 *
...
try {
    ...
    $user = $parameters['solaris_zfs_scrub_check_user'];
    $port = $parameters['solaris_zfs_scrub_check_port'];
    
    $ip = $job_infos['ip'];
    
    $ssh    = new SSHUtil();
    $path   = dirname($argv[0]);
    $script = $path . '/zfs-scrub-check.ksh';
