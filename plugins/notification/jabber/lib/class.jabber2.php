 ...
 * @notes: (2007-12-06) The lib was adapted for work with Ortro, now when an error occured an exception is throwned
...
class Jabber
{
    ...
    function Jabber($login, $password, $security = SECURITY_NONE, $port = 5222, $host = '')
    {
        ...
        if (!extension_loaded('xml')) {
            $error_msg = 'Error: No XML functions available, Jabber functions can not operate.';
            $this->log($error_msg);
        ...
        if ((version_compare(phpversion(), '5.2.1', '>=') && version_compare(phpversion(), '5.2.3RC2', '<')) && $security != SECURITY_NONE) {
            $error_msg = 'Error: PHP ' . phpversion() . ' + SSL is incompatible with jabber, see http://bugs.php.net/41236';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }

        if (!Jabber::check_jid($login)) {
            $error_msg = 'Error: Jabber ID is not valid: ' . $login;
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    ...
    function send($xml)
    {
        if ($this->connected()) {
           ...
           return fwrite($this->connection, $xml);
            ...
            $error_msg = 'Error: Could not send, connection lost (flood?).';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    }
    ...
    function login()
    {
        if (!count($this->features)) {
            $error_msg = 'Error: No feature information from server available.';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    ...
    function register()
    {
        if (!isset($this->session['id']) || isset($this->session['jid'])) {           
            $error_msg = 'Error: Cannot initiate registration.';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    ...
    {
        if (!isset($this->session['id']) || !isset($this->session['jid'])) {
            $error_msg = 'Error: Cannot initiate un-registration.';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    ...
    function presence($type = '', $message = '', $unavailable = false)
    {
        if (!isset($this->session['jid'])) {
            $error_msg = 'Error: Cannot set presence at this point.';
            $this->log($error_msg);
            throw new Exception($error_msg);
        }
    ...
    function response($xml)
    {
        ...
        switch (key($xml)) {
            ...
            case 'stream:features':
                ...
                if (isset($xml['stream:features'][0]['#']['mechanisms'][0]['@']['xmlns']) &&
                    ...
                    // not good...
                        ...
                        $error_msg = 'Error: No authentication method supported.';
                        $this->log($error_msg);
                        $this->disconnect();
                        throw new Exception($error_msg);
                    }
                    ...
                    // ok, this is it. bye.
                    $this->log($error_msg);
                    $error_msg = 'Error: Server does not offer SASL authentication.';
                    $this->disconnect();
                    throw new Exception($error_msg);
                }
                break;
            ...
            case 'failure':
                $error_msg = 'Error: Server sent "failure".';
                $this->disconnect();
                $this->log($error_msg);
                throw new Exception($error_msg);
            ...
            case 'proceed':
                ...
                if (!stream_socket_enable_crypto($this->connection, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)) {
                    $error_msg = 'Error: TLS mode change failed.';
                    $this->log($error_msg);
                    throw new Exception($error_msg);
                }
