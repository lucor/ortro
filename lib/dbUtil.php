...
class DbUtil
{
   ...
   /**
     ...
     * @return string the sql statement
     */
    function setWorkflow($label, $description, $status, $id_system)
    {
        $stmt = 'insert into ' . $this->setTablePrefix('workflows') . 
                " (label,description,status,id_system) values ('" .  
     ...
     * @return string the sql statement
     */
    function updateWorkflow($id_workflow,$label,$description)
    {
    ...
    /**
     * SQL query to set a workflow step
     * 
     * @param int    $id_workflow     the wokflow id
     * @param int    $id_job          the job id
     * @param int    $step            the step to update
     * @param int    $on_success_step the step to execute on success
     * @param string $on_success_when define when execute the step on success
     * @param int    $on_error_step   the step to execute on success
     * @param string $on_error_when   define when execute the step on success
     * @param string $status          the status
     * 
     * @return string the sql statement
     */
    function setWorkflowStep($id_workflow, $id_job, $step, $on_success_step, 
                             $on_success_when, $on_error_step, 
                             $on_error_when, $status)
    {
        $stmt = 'insert into ' . $this->setTablePrefix('workflows_step') . 
                ' (id_workflow,id_job,step,on_success,on_success_when,on_error,' .
                'on_error_when,status) values (' . 
                $id_workflow . "," .
                $id_job . "," .
                $step . "," .
                $on_success_step . ",'" .
     ...
     * @return string the sql statement
     */
    function updateWorkflowStepWorkflow($id_workflow, $step, $on_result, 
                                        $go_to_step, $when)
    {
        $stmt = 'update ' . $this->setTablePrefix('workflows_step') . ' set ' .
                $on_result . '=' . $go_to_step . ',' . 
                $on_result . "_when='" . $when . "'" .
                ' where id_workflow=' . $id_workflow . 
                ' and step=' . $step;
        return $stmt;
    }
     ...
     * @return string the sql statement
     */
    function updateWorkflowStepStatus($id_workflow, $step, $status)
    {
        $stmt = 'update ' . $this->setTablePrefix('workflows_step') . ' set ' .
                "status='" . $status . "'" .
                ' where id_workflow=' . $id_workflow; 
    ...
    /**
     * SQL query to check for step dependencies  
     * 
     * @param int $id_workflow workflow id 
     * @param int $step        step
     * 
     * @return string the sql statement
     */
    function checkWorkflowStep($id_workflow, $step)
    {
        $stmt = 'select * ' . 
                'from ' . $this->setTablePrefix('workflows_step') .
                ' where (on_error != 0 or on_success != 0) ' .
                ' and id_workflow=' . $id_workflow . 
                ' and step=' . $step;
        return $stmt;
    }
     ...
     * @return string the sql statement
     */
    function checkWorkflowStepInfoByIdJob($id_job)
    {
        $stmt = 'select * ' . 
                'from ' . $this->setTablePrefix('workflows_step') .
                ' where id_job=' . $id_job . 
    ...
    /**
     * SQL query to delete worklow step
     * 
     * @param int $id_workflow workflow id     
     * @param int $step        step
     * 
     * @return string the sql statement
     */
    function deleteWorkflowStep($id_workflow, $step)
    {
        $stmt = 'delete from ' . $this->setTablePrefix('workflows_step') . 
                ' where id_workflow=' . $id_workflow;
        if ($step != '*') {
                $stmt .= ' and step=' . $step;            
        }
    ...
    function updateJob($jobId, $shdId, $jobTypeId, $label, $description, $priority, 
                       $properties, $parameters='', $identity=0)
    {
     ...
     * 
     * @param int    $id_workflow workflow id
     * @param int    $step        the step id
     * @param string $status      the step status
     * 
     * @return string the sql statement
     */
    function getWorkflowInfoById($id_workflow, $step, $status='')
    {
        $stmt = 'select * ' .
                ' from ' .
                $this->setTablePrefix('workflows_step') . 
    ...
    */
    function getJobsWorkflowToExec($available_shd)
    {
        ...
        $stmt = 'select distinct a.id_job, b.step, c.label, '.
                'c.category, b.id_workflow ' .
                'from ' . $this->setTablePrefix('jobs') . ' as a, ' .
                          $this->setTablePrefix('workflows_step') . ' as b, ' .  
                          $this->setTablePrefix('job_type') . ' as c, ' .
                          $this->setTablePrefix('job_crontab') . ' as d, ' .
    ...
    */
    ...
    function getJobInfoById($jobId)
    {
        $stmt = 'select ' .
                ...
                'a.identity, a.properties, a.id_job_type, b.id_system, b.id_host,'.
