...
class CronUtil
{
    ...
    function cronUtil($requestType = 'CRONTAB')
    {
        ...
        $this->wf_logger      = new LogUtil('cronUtil.php', ORTRO_LOG.'workflow');
    ...
    function runCron()
    {
        ...
        $rows_workflows = $dbUtil->dbQuery($dbh,
                                           ...
                                           MDB2_FETCHMODE_ASSOC);
        $executed_jobs  = array();
        $rows_workflows_to_exec = $rows_workflows + $rows_job_workflows;
        foreach ($rows_workflows_to_exec as $key=>$row) {
            $job_id_executed = $this->execWorkflowNode($row['id_workflow'], $row['id_node']);
            //Add the job to $executed_jobs array
            array_push($executed_jobs, $job_id_executed);
        }
    ...
    function execWorkflowNode($id_wf, $id_node) {
        $dbUtil = new DbUtil();
        
        $dbh  = $dbUtil->dbOpenConnOrtro();
        $rows = $dbUtil->dbQuery($dbh,
                                 $dbUtil->getWorkflowNodeInfo($id_wf, $id_node),
                                 MDB2_FETCHMODE_ASSOC);
        
        $this->wf_logger->trace('DEBUG', print_r(array($id_wf, $id_node),true));
        ...
        $dbUtil->dbExec($dbh,
                        $dbUtil->updateWorkflowStatus($id_wf,
                        'R'));
        //Update the step status for this workflow as undefined
        $dbUtil->dbExec($dbh,
                        $dbUtil->updateWorkflowNodeStatus($id_wf,
                        '*',
                        '-'));
        ...
        $log_prefix = '[workflow: ' . $node['id_workflow'] .
                      '][node: ' . $node['id_node'] .
                      '][id_to_exec: ' . $node['id_to_exec'] . ']';
                      '][type_to_exec: ' . $node['type_to_exec'] . ']';

        $this->wf_logger->trace('DEBUG', print_r($node,true));
        $this->wf_logger->trace('DEBUG',
                             $log_prefix .
                             '[mode: crontab] workflow started...');

        switch ($node['type_to_exec']) {
            case 'J':
                $job_id = $node['id_to_exec'];
                $job_info = $dbUtil->dbQuery($dbh,
                                             $dbUtil->getJobType($job_id),
                                             MDB2_FETCHMODE_ASSOC);

                //Execute the job
                $this->execJob($job_id, $job_info[0]['label'], $job_info[0]['category']);
                unset($job_info);
                break;
            case 'W':
                //getWorkflowInfoById($row['id_to_exec'], 0);
                         ...
                         $attachments, $dynamic_params='', $enable_notification=true)
    {
        ...
        $dbUtil = new DbUtil();
        $this->logger->trace('DEBUG', 'Result for job ' . $id_job . ' -> ' . $result);
        $this->logger->trace('INFO', 'Job ' . $pluginName . ' with id=' . $id_job . ' done.');
        switch ($this->requestType) {
        case 'XML-RPC':
        ...
        $workflowNode = $dbUtil->dbQuery($dbh,
                                         $dbUtil->checkWorkflowNodeInfoByIdJob($id_job),
                                         MDB2_FETCHMODE_ASSOC);

        if (count($workflowNode) != 0) {
            //$this->logger->trace('DEBUG', print_r($workflowNode, true));
            //The job is a step of at least a workflow
            
            $id_workflow = $workflowNode[0]['id_workflow'];
            $id_node     = $workflowNode[0]['id_node'];
            $log_prefix  = '[workflow: ' . $id_workflow . ']';

            $this->wf_logger->trace('DEBUG', $log_prefix . '[id_node: ' . $id_node . '] Done.');

            // Update the step status in accord to $result
            $dbUtil->dbExec($dbh,
                            $dbUtil->updateWorkflowNodeStatus($id_workflow,
                                                              $id_node,
                                                              $result));
            $exec_condition = 0;
            if ($result == '0') {
            ...
            $this->wf_logger->trace('DEBUG', $log_prefix . '[id_node: ' . $id_node . '] Check for child node to execute...');

            $nodes_to_exec = $dbUtil->dbQuery($dbh,
                                              $dbUtil->getWorkflowInfoById($id_workflow,
                                                                           $id_node,
                                                                           $exec_condition),
                                              MDB2_FETCHMODE_ASSOC);

            if (count($nodes_to_exec) != 0 ) {
                //There is a next step
                
                $log_prefix .= '[id_parent_node: ' . $id_node . ']';
                $this->wf_logger->trace('DEBUG', print_r($nodes_to_exec, true));
                $this->wf_logger->trace('DEBUG', $log_prefix . ' Nodes found');

                foreach ($nodes_to_exec as $node) {
                    switch ($node['exec_properties']) {
                        case 'W':
                            //Wait for define schedule
                            //Update the status of the node
                            $this->wf_logger->trace('DEBUG', $log_prefix . '[id_node: ' . $node['id_node'] . '] Waiting for schedule');
                            $dbUtil->dbExec($dbh,
                                            $dbUtil->updateWorkflowNodeStatus($node['id_workflow'],
                                            $node['id_node'],
                                            'W'));
                            break;
                        case 'I':
                            //Run immediately
                            //Update the status of the node
                            $this->wf_logger->trace('DEBUG', $log_prefix . '[id_node: ' . $node['id_node'] . '] Executing immediately');
                            $dbUtil->dbExec($dbh,
                                            $dbUtil->updateWorkflowNodeStatus($node['id_workflow'],
                                            $node['id_node'],
                                            'R'));
                            ...
                            switch ($node['type_to_exec']) {
                                ...
                                case 'W':
                                    ...
                                    $id_wf = $node['id_to_exec'];
                                    $wf_info_root_node = $dbUtil->dbQuery($dbh,
                                                                $dbUtil->getWorkflowInfoById($id_wf, '0'),
                                                                MDB2_FETCHMODE_ASSOC);
                                    $id_root_node = $wf_info_root_node[0]['id_node'];
                                    $this->execWorkflowNode($id_wf, $id_root_node);
                                    break;
                                default:
                                    //Strange value... exit
                                    continue;
                ...
                $this->wf_logger->trace('DEBUG', $log_prefix . '[id_node: ' . $id_node . '] has no child.');
                $this->wf_logger->trace('DEBUG', $log_prefix . 'Checking for running/waiting nodes...');

                $active_nodes = $dbUtil->dbQuery($dbh,
                                                 $dbUtil->checkWorkflowRunningNodes(),
                                                 MDB2_FETCHMODE_ASSOC);
                if(count($active_nodes) == 0) {
                    //There are not other nodes to execute... workflow completed!
                    $this->wf_logger->trace('DEBUG', $log_prefix . 'Workflow completed.');
                    $dbUtil->dbExec($dbh,
                                    $dbUtil->updateWorkflowStatus($id_workflow, 'W',time())
                                   );
                } else {
                    $this->wf_logger->trace('DEBUG', $log_prefix . 'There are active nodes...');
                    $this->wf_logger->trace('DEBUG', $log_prefix . print_r($active_nodes,true));
                }
        }
